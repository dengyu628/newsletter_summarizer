...
def fetch_unread_email_dates_and_update_ui(progress=gr.Progress()):
    ...
    try:
        ...
        default_date_obj = sorted_dates[0][0] if sorted_dates else yesterday_obj
        default_start_dt = datetime.combine(default_date_obj, datetime.min.time())
        default_end_dt = datetime.combine(default_date_obj, datetime.max.time())
        
        progress(1, desc="✅ 日期解析完毕！")
        return "✅ 日期解析完毕！请在下方选择日期范围。", gr.update(choices=formatted_choices, value=formatted_choices[0], interactive=True), gr.update(value=default_start_dt, interactive=True), gr.update(value=default_end_dt, interactive=True), gr.update(interactive=True)
